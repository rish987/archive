...
\begin{part}{Notation}
  ...
  Let's denote
  $$\text{$\S :=$ the set of \til{}s},$$
  and 
  $$\text{$\A :=$ the set of \act{}s.}$$
  Assume that $\S$ and $\A$ are finite.
  Denote
  $$\text{$\RW : \S \rightarrow \R :=$ mapping from \til{}s to \rwd{}s.}$$

  Denote
  $$\text{$\T :=$ the \lmt, where $\T \ge 1$},$$
  $$\text{$\STT = \left(\edots\STT{1}{\T}\right) :=$ an arbitrary \str, 
  where each mapping $\STT_\TT : \S \rightarrow \A$},$$
  and
  $$\text{$\ST :=$ the set of all \strs\ $\pi$}.$$
  Note that $|\ST| = \T \times |\S| \times |\A|.$

  Now, suppose the \agt\ is following some \str\ $\STT$.
  For all $1 \le \TT \le \T$,
  denote
  $$\text{$\S_\TT :=$ random variable for the \til\ at \stp\ $\TT$},$$
  and
  $$\text{$\A_\TT :=$ random variable for the \act\ at \stp\ $\TT$},$$
