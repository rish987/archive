\def\rltpath{archive/topic/pr/topic/rl}
...
\def\whenfollow{when the \agt\ follows a \str\ $\astr$}
  ...
  \def\condprobpath{archive/topic/pr/definition/cprob}
  \def\margprobpath{archive/topic/pr/proof/sqmprobgexpr}

\def\aprod#1#2#3{\enm{\prod_{#1}^{#2}#3}}

\def\varsellist{\SS_1, \AA_1, \SS_2, \ldots, \SS_\T}
\def\vardomlist{(\S \times \A)^{\T - 1} \times \S}
