...
\begin{part} {Physical intuition}
  ...
  Let's call this ratio the ``\prb'' of the \jasn.
  Let's refer to such a \seq\ as a \pseq.

  \newpage
  For example, if the \seq\ above repeated the first five \trl{}s, it would be a \pseq\ defining the \prbs:
  \begin{center}
    \begin{tikzpicture}
      \tjasn{A1}{B1}{\B}{\1}{(0,0)}
      ...
      \node[anchor=south] at (A1.north) {\prb: 0.4};
      \node[anchor=south] at (A2.north) {\prb: 0.2};
      \node[anchor=south] at (A3.north) {\prb: 0.4};
      \node[anchor=south] at (A4.north) {\prb: 0.0};
    \end{tikzpicture}
  \end{center}

  For any assignment of \prbs\ to all possible \jasn{}s,
  let a ``\dist'' be the set of all \seq{}s
  that define those \prbs.
\end{part}
